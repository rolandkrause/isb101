...
<span class = "small show">[Wickham H. 2007. _J. Comp. Graph. Stat._ **19**:3–28](http://www.tandfonline.com/doi/abs/10.1198/jcgs.2009.07098)</span>


## Simple example | Wickham 2007 {.build}

### dataset {.box-4 .bg-green}
...
### 3 layers combined {.box-6 .bg-green}
...
### Faceting (_aka_ trellis or lattice plots) {.box-8 .offset-2 .bg-grgold}

What if we want to split _circles_ and _squares_?

## Faceting  {.build}

### Split by the **shape** {.box-8 .offset-2 .bg-blue-white}
...
Now, **dot shapes** and **facets** provide the same information. Shapes could be freed for another meaningful variable
...
## Iris dataset | rstudio [cheatsheet](https://www.rstudio.org/links/data_visualization_cheat_sheet)

```{r, title = "save as `tibble` the long `data.frame`", class = "compact-output"}
iris <- as_tibble(iris)
iris
```

### smart printing of the iris dataset {.box-6 .bg-yellow .stretch}
...
```{r, fig.height = 4, title = "Petal: Length versus Width"}
iris %>%
  ggplot() +
...
## Mapping aesthetics {.vs2}

### definitions {.box-12 .bg-gray}

- **aesthetics** map the columns of a `data.frame`/`tibble` to the variable each ggplot2 `geom` is expecting.
- example `geom_point()` requires **at least** the _**x**_ and _**y**_ coordinates for each point.
...
```{r, results = "asis", echo = FALSE, class = "compact-table"}
iris %>%
  head(5) %>%
  knitr::kable()
```
...
### problem, toy data {.box-6 .bg-red .stretch}
...
## Stat functions {.nvs1 .build}

### transform data {.bg-aquamarine2 .box-12 .icon}
...
- `geom` uses a `stat` function to transform the data:
    + `geom_histogram()` uses `stat_bin()` divide and count values into _bins_
...
<span class = "small">source: ggplot2 cheatsheet by Rstudio</span> 
...
- `geom_bar()` uses `stat_count()` to compute (creating a new `count` column)
...
## Barcharts | categorical variables {.build}

### map a continous variable on `y` axis{.box-12 .bg-gray}

-  we need to override the default call to `stat_count()`
- `stat = "identity"` will force `geom_bar()` to use `stat_identity()` instead
