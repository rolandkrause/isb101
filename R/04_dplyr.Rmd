...
- Was downloaded from bioMart and saved as a `.csv` and provided on Moodle.
...
Using the `tibble` stored in `gene_by_exon`, provide an overview of some intersting functions provided by `dplyr`
...
- Shows some values and types per column. 
- The *Environment* tab in RStudio tab does it too (expand it clicking on the button left to your object `gene_by_exon`)
...
### multiple conditions: AND {.box-8 .offset-2 .bg-blue}
...
### multiple conditions: OR {.box-8 .offset-2 .bg-blue}
...
### `arrange()` {.box-12 .bg-yellow .compact-output}

Can even perform a nested sorting:
...
gene_by_exon %>%
  mutate(length = end_position - start_position,
         codons = length %% 3 == 0) %>% 
  select(hgnc_symbol, length, codons)  
```
...
gene_by_exon %>%
  select(ensembl_gene_id, start = start_position, end = end_position) %>% 
  distinct() %>% 
  arrange(start) %>% 
