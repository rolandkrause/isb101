...
### Learning objectives {.box-8 .offset-2 .bg-red}
...
- Get to know a set of packages for easy data manipulation and visualization (*tidyverse* including *dplyr* and *ggplot*)
...
- R5 Visualization (Thu afternoon)
- R6 Genomics in R (Fri morning)


## How to install R on my PC?

R is installed on your computers already, this is for reference.
...
- _Autocompletion_ using <kbdTAB</kbd>
...
### Four main types {.box-6 .bg-blue-white}
...
### Special types {.box-6 .bg-blue-white .stretch .build}
...
### Integers {.box-8 .offset-2 .bg-blue-white}
...
### Double {.box-8 .offset-2 .bg-blue-white}
...
### `double` is the default {.box-6 .bg-blue-white .stretch}
...
### Factors {.box-6 .stretch .bg-blues}
...
### Matrix and Arrays {.box-6 .bg-blue .stretch}
...
```{r, dataframe, row = TRUE, collapse = TRUE}
L3 <- LETTERS[1:3]
...
```{r, title = "Example of assignment"}
x <- 7
```


```{r, title="Create a new variable based on <b>a</b>" }
a <- 2
c <- (a^5 - a)
c
```
...
#### Stay consistent {.box-6 .bg-yellow}

- Choose a [naming convention](https://journal.r-project.org/archive/2012-2/RJournal_2012-2_Baaaath.pdf):
- alllowercase
- period.separated
...
- Change the column names of data frame using `colnames()`
    ```{r}
...
### What is a function? Tip{.box-10 .offset-1 .bg-yellow .icon-small}
