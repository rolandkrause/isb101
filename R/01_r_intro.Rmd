...
author: "G. Androsova, E. Koncina, A. Ginolhac, R. Krause"
date: "2018-01-08"
output: 
  iosp::ioslides_plus:
    box_colours:
...
```{r, include = FALSE}
library(tidyverse)
swirl_mailto <- "roland.krause@uni.lu" #"eric.koncina@uni.lu"
```
...
### {.box-8 .offset-2 .bg-white .img-center}

![](img/01/rstudio_repro.png)
...
### Adjust settings {.box-6 .bg-blue-white .small}

![](img/01/diag_rstudio.png)
source: [Kevin Ushey' article](https://support.rstudio.com/hc/en-us/articles/205753617-Code-Diagnostics?version=1.1.133&mode=desktop)

## Code diagnostics

### check argument calls {.box-6 .bg-blue-white}

![](img/01/rstudio_check_args.png)


###  missing arguments {.box-6 .bg-blue-white}

![](img/01/rstudio_check_missing.png)

### variable definitions {.box-6 .offset-3 .bg-blue-white}

![](img/01/rstudio_check_vars.png)

- unused variables & style recommendations

## Getting started {.bg-green}

### Use **R** and **RStudio** {.box-8 .offset-2 .bg-blue-white}
...
### **Remember** that R will: {.box-10 .offset-1 .bg-yellow .icon-small}
...
### Inputs may be {.box-8 .offset-2 .bg-blue}

- numbers (_e.g._ 1, 5652654, 163.0056 ...)
- letters (_e.g._ "x", "My agenda" or "Anny likes John")
- logical expressions (_e.g._ "if time < 10 am then I'll stay in bed!")
- matrices/tables

## Data types in R

### 4 main types {.box-6 .bg-blue-white}
...
```{r, eval = FALSE, title = "Special case", width = 6}
...
## Structures {.nvs1 .build}

### Vectors {.box-6 .stretch .bg-yellow}
...
```{r, row = TRUE, class = "compact-output"}
...
### Matrix and Arrays {.box-6 .bg-green .stretch}
...
### Assignment {.box-6 .bg-yellow}

In R you create **`objects`** to store anyting by a process called **assignment**.

%end%

```{r, title = "Example of assignment", width = 6}
x <- 7
```

### Warning {.box-6 .offset-3 .bg-red .icon-small}
...
### Guess {.box-6 .bg-green .build .middle}
...
```{r, title = "Basic R operations", width = 6}
a * 4
sqrt(a)
exp(a)
log(a)
...
- use `sort()` to arrange elements in increasing order or `order()` to make more complex sorting.

### Mini-task{.box-6 .offset-3 .bg-green}

How would you arrange your vector in a decreasing order using `sort()`?

## Operations with vectors {.bg-green .build}

### Help{.box-8 .offset-2 .bg-blue-white .show}
...
```{r, title = "Solution", width = 6, class = "offset-3"}
...
```{r, title = "Example", width = 6, class = "show"}
v = c("Mary", "Sue")
...
### Retrieve elements by names {.box-6 .bg-yellow}

- We can retrieve the first member by its name.
    ```{r}
    v["First"] 
    ```
...
my_list <- list(name = "Fred", my_numbers = a, my_matrix = c, age = 5.3)

# example of a list containing two lists 
two_lists <- list(my_list, list("first_element", "second_element"))
```

## Lists

Extract elements of a list using the `[[]]` convention
```{r}
my_list[[1]] # 1st component of the list
my_list[["my_numbers"]] # component named mynumbers in list
```
...
<!-- Add salt and pepper example... -->
<!-- Using `[]` with `[[]]` above might be confusing... -->
...
### Coercion {.box-6 .offset-3 .bg-blue-white .show}
...
- Change the column names of data frame using `names()`
    ```{r}
...
```{r, title = "Create your custom functions", width = 6, class = "offset-3 show"}
myfun <- function(x){ 
  x*x
}
...
### Example 2{.box-10 .offset-1 .bg-blue-white}
...
## The `apply()` family of commands {.build}
...
### Compare vectorised and non-vectorised `log()` {.box-12 .bg-blue-white}
...
### Vectorised conditionals {.box-8 .offset-2 .bg-blue}
...
### Vectorised conditionals {.box-6 .bg-blue}

`ifelse()`

### Default vectorised matrix functions {.box-6 .bg-blue}

`rowSums()`, `colSums()`, `rowMeans()`, `colMeans()`

### Vectorized matrix functions in package genefilter {.box-6 .bg-blue}

`rowSds()`, `colSds()`, `rowVars()`, `colVars()`, `rowttests()`, `rowFtests()`

### Vectorized matrix functions in package matrixStats {.box-6 .bg-blue}
