...
For filtering it is recommned to use the `str_detect()` function from the *stringr* package included in the *tidyverse*.
...
Load the file `corpasome.trio.chr22.snps.filtered.annovar.tsv` into R.
 1. Filter the columns `corpasome_dad`, `corpasome_mom`and `corpasome_daughter` with the patterns $01$, $01$, and $11$ respectively. Compare your results to the file `corpasome.trio.chr22.snps.filtered.annovar.recessive`.
...
```
 ...
 2. Filter the columns `esp6500si_ea`,`1000g2012apr_eur` such that the values above $0.05$ in any column are removed. Be careful about missing values in this column, they need to be included!
 
Compare to the file named `corpasome.trio.chr22.snps.filtered.annovar.recessive.noncommon`.
...
```
 ...
 3. From the results above filter on the two rows `Func.refGene` and `ExonicFunc.refGene`. 
 
  * In `Func.refGene`, filter rows such that  
  *splicing* or *exonic* remains and 
...
```
  4. Now, we want to filter out the deleterious variants. Remove the patterns *D* and *P* from each of the columns `ljb2_sift` and `ljb2_pp2hvar` . Compare to `corpasome.trio.chr22.snps.filtered.annovar.recessive.noncommon.exonsplicing.pred`.
...
```
  ...
  4. From the file `corpasome.trio.chr22.denovogear.annovar_input.hg19_multianno.txt.noncommon..notdbSNP`, alter two rows `Func.refGene` and `ExonicFunc.refGene`. In `Func.refGene`, filter rows such that  *splicing* or *exonic;splicing* remains and in `ExonicFunc.refGene` column, *frameshift* or *nonsynonymous* or *stopgain* or *stoploss* is included. Save all of these changes into the file `corpasome.trio.chr22.denovogear.annovar_input.hg19_multianno.txt.noncommon.notdbSNP.exonsplicing`
  
```{r, solution = TRUE}
dbsnp %>% 
  filter(str_detect(Func.refGene, "exonic")) %>% 
  filter(str_detect(ExonicFunc.refGene, c("nonsynonymous|frameshift|stopgain|stoploss"))) -> exonsplicing
  
```
  
  5. Now, we want to filter out the deleterious variants. Remove the patterns `D` and `P` from each of the columns ljb2_sift,ljb2_pp2hvar . Rename this new file to corpasome.trio.chr22.denovogear.annovar_input.hg19_multianno.txt.noncommon.notdbSNP.exonsplicing.pred
```{r, solution = TRUE}
exonsplicing %>% 
#  filter( !str_detect(ljb2_sift, "D|P")) %>% 
 filter(          str_detect(ljb2_pp2hvar, "D|P")) -> pred
```
...
```{r, solution = TRUE}
semi_join(pred, cand_genes, by=c(Gene.refGene = "X1"))
```
...
dad_cov_stats <- read_tsv(file.path(coverage_dir, "dad.coverage", colnames(c("chr", "start", "stop", "cov", "N", "P", "Q"))))
```
