...
Load the file `corpasome.trio.chr22.snps.filtered.annovar.tsv` into R.
 1. Filter the columns `corpasome_dad`, `corpasome_mom`and `corpasome_daughter` with the patterns $01$, $01$, and $11$ respectively. Compare your results to the file `corpasome.trio.chr22.snps.filtered.annovar.recessive`. Why are these patterns particularly important in this case ?
...
```
 ...
 2. Filter the columns `esp6500si_ea`,`1000g2012apr_eur` such that the values above $0.05$ in any row are removed. Be careful about missing values in this column, they need to be included!
...
```
  ...
  5. Now, we want to filter for the deleterious variants. Remove other patterns such that only  `D` from each of the columns ljb2_sift,ljb2_pp2hvar remains. Rename this new file to corpasome.trio.chr22.denovogear.annovar_input.hg19_multianno.txt.noncommon.notdbSNP.exonsplicing.pred
```{r, solution = TRUE}
exonsplicing %>% 
#  filter( !str_detect(ljb2_sift, "D|P")) %>% 
