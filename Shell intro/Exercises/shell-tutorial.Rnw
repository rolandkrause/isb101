...
First we need the human knowledge dataset from \href{http://compartments.jensenlab.org/Downloads}{http://compartments.jensenlab.org/Downloads}. This file contains human subcellular localization information from model organisms databases (mostly from UniProt). It can be be downloaded:
...
We can also get the non-mitochondrial proteins. In the \verb+grep+ command using \verb+-v+ flag lists all lines, which do not contain the pattern. One example:

\begin{verbatim}
grep -v 'GO:0005739' human_compartment_knowledge_full.tsv | head
\end{verbatim}

We will store the non-mitochondrial proteins as well.
